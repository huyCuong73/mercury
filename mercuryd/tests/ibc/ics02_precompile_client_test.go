...
import (
	...
	"github.com/huyCuong73/mercury/precompiles/ics02"
	evmibctesting "github.com/huyCuong73/mercury/testutil/ibc"
...
func (s *ICS02ClientTestSuite) SetupTest() {
	...
	evmAppA := s.chainA.App.(*mercuryd.EVMD)
	s.chainAPrecompile = ics02.NewPrecompile(
		evmAppA.AppCodec(),
		evmAppA.IBCKeeper.ClientKeeper,
	)
	evmAppB := s.chainB.App.(*mercuryd.EVMD)
	...
	testCases := []struct {
	...
	}{
		{
			...
			malleate: func() {
				...
				clientState, found := s.chainA.App.(*mercuryd.EVMD).IBCKeeper.ClientKeeper.GetClientState(
					s.chainA.GetContext(),
					clientID,
				)
	...
	testCases := []struct {
	...
	}{
		{
			...
			malleate: func() {
				...
				trustedHeight := s.chainA.App.(*mercuryd.EVMD).IBCKeeper.ClientKeeper.GetClientLatestHeight(
					s.chainA.GetContext(),
					clientID,
				)
		...
		},
		{
			...
			malleate: func() {
				...
				err = s.chainA.App.(*mercuryd.EVMD).IBCKeeper.ClientKeeper.UpdateClient(s.chainA.GetContext(), clientID, header)
				s.Require().NoError(err)
		...
		},
		{
			...
			malleate: func() {
				...
				height := s.chainA.App.(*mercuryd.EVMD).IBCKeeper.ClientKeeper.GetClientLatestHeight(
					s.chainA.GetContext(),
					clientID,
				)
	...
	testCases := []struct {
	...
	}{
		{
			...
			malleate: func() {
				...
				timestampNano, err := s.chainA.App.(*mercuryd.EVMD).IBCKeeper.ClientKeeper.GetClientTimestampAtHeight(s.chainA.GetContext(), clientID, proofHeight)
				s.Require().NoError(err)
				...
				err = s.chainA.App.(*mercuryd.EVMD).IBCKeeper.ClientKeeper.VerifyMembership(
					s.chainA.GetContext(),
					clientID,
					proofHeight,
					0,
	...
	testCases := []struct {
	...
	}{
		{
			...
			malleate: func() {
				...
				timestampNano, err := s.chainA.App.(*mercuryd.EVMD).IBCKeeper.ClientKeeper.GetClientTimestampAtHeight(s.chainA.GetContext(), existingClientID, proofHeight)
				s.Require().NoError(err)
				...
				path := commitmenttypesv2.NewMerklePath(pathBz...)
				err = s.chainA.App.(*mercuryd.EVMD).IBCKeeper.ClientKeeper.VerifyNonMembership(
					s.chainA.GetContext(),
					existingClientID,
					proofHeight,
					0,
