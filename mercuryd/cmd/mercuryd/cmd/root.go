...
import (
	...
	cosmosevmcmd "github.com/huyCuong73/mercury/client"
	mercurydebug "github.com/huyCuong73/mercury/client/debug"
...
func NewRootCmd() *cobra.Command {
	...
	tempApp := mercuryd.NewExampleApp(
		log.NewNopLogger(),
		dbm.NewMemDB(),
		nil,
		true,
...
func initRootCmd(rootCmd *cobra.Command, evmApp *mercuryd.EVMD) {
	cfg := sdk.GetConfig()
	cfg.Seal()
...
func appExport(
	...
	if height != -1 {
		exampleApp = mercuryd.NewExampleApp(logger, db, traceStore, false, appOpts, baseapp.SetChainID(chainID))

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = mercuryd.NewExampleApp(logger, db, traceStore, true, appOpts, baseapp.SetChainID(chainID)) // TODO:VLAD - Remove // TODO:VLAD - Remove appoptions and evmchainid
	}
