...
add_genesis_funds() {
  local keyname="$1"
  mercuryd genesis add-genesis-account "$keyname" 1000000000000000000000aplab --keyring-backend "$KEYRING" --home "$CHAINDIR"
}
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  mercuryd genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000aplab --keyring-backend "$KEYRING" --home "$CHAINDIR"
  ...
  # --------- Finalize genesis ---------
  mercuryd genesis gentx "$VAL_KEY" 1000000000000000000000aplab --gas-prices ${BASEFEE}aplab --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
